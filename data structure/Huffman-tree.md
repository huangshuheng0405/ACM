...
### 从根节点到各叶节点的路径长度于相应的叶节点权值的乘积之和称为树的带权路径长度（**Weighted Path Length of Tree,WPL**)

#### 设$w_i$为二叉树第i个叶节点的权值，$l_i$为从根节点到第i个叶结点的路径长度，则WPL计算公式为
...
int main(){
    ...
    while((q.size()-1)%(k-1)!=0)//如果最后一次合并节点，但个数不足k个，最靠近根节点的位置没被排满
        q.push({0,1});//利用空结点把其他结点挤到更优的位置上（先排权值较低的点）
    while(q.size()>=k){
        ll h1=-1,sum=0;
        for(int i=1;i<=k;i++){
            node t=q.top();
        ...
        ans+=sum;
        q.push({sum,h1+1});//将新合并的结点在压入队列中
    }//最长字符串的最短长度就是最深的结点的长度
